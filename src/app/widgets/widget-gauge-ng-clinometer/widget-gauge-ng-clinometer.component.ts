...
@Component({
  selector: 'app-widget-gauge-ng-clinometer',
  templateUrl: './widget-gauge-ng-clinometer.component.html',
  styleUrls: ['./widget-gauge-ng-clinometer.component.css'],
  standalone: true,
  imports: [AngularResizeEventModule, GaugesModule, AsyncPipe]
})
export class WidgetGaugeNgClinometerComponent extends BaseWidgetComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('ngClinometerWrapperDiv', { static: true, read: ElementRef }) wrapper: ElementRef;
  @ViewChild('clinometerGauge', { static: true }) clinometerGauge: RadialGauge;
  ...
  // fix for ClinometerGauge GaugeOptions object ** missing color-stroke-ticks property
  ...
  constructor() {
    ...
    this.defaultConfig = {
      ...
      gauge: {
        type: 'ngClinometer', // capacity, measuring, marineCompass, baseplateCompass
  ...
  ngAfterViewInit(): void {
    this.clinometerGauge.update(this.gaugeOptions);
    ...
    this.observeDataStream('gaugePath', newValue => {
      ...
      if (this.state !== newValue.state) {
        ...
        //@ts-ignore
        let option: ClinometerGaugeOptions = {};
        ...
        this.clinometerGauge.update(option);
      }
    });
  ...
  public onResized(event: ResizedEvent): void {
    if (!event.isFirst) {
      //@ts-ignore
      let resize: ClinometerGaugeOptions = {};
      ...
      this.clinometerGauge.update(resize);
    }
  }
  ...
  private configureCapacityGauge(): void {
    ...
    this.gaugeOptions.startAngle = 90;
  ...
  private configureMeasuringGauge(): void {
    ...
    this.gaugeOptions.ticksAngle = 365;
  ...
  private setHighlights(zones: ISkZone[]): void {
    ...
    this.clinometerGauge.update(highlights);
  }
